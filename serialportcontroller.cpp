...
serialPortController::serialPortController(QString serialConnection, std::queue<std::map<QByteArray, QString, int>> serialWriteQueue, std::queue<float> serialReadQueue)
{
...
void serialPortController::run()
{
    ...
    */
    ...
    while (true) {
        ...
        float message = 1;//  this->serialWriteQueue.front();     // TODO
        // If the first part of the message is None this signals the process to close the port and stop
        if(message == 0) { // TODO
            break;
        }
